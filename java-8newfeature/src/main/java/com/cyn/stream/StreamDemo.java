...
public class StreamDemo {
    public static void main(String[] args) {
        ...
        strings.stream().map(string -> string + "Map").forEach(string -> System.out.println(string));
        System.out.println("===============");

        // 4.filter 用于通过设置的条件过滤出元素
        strings.stream().filter(s -> s.contains("b")).forEach(s -> System.out.println(s));
        System.out.println("===============");
        ...
        strings.parallelStream().filter(s -> s.contains("c")).forEach(s -> System.out.println(s));
        System.out.println("===============");
        ...
        List<String> filteredss = strings.stream().filter(string -> !string.isEmpty()).collect(Collectors.toList());
